...
for ii = 1:size(cities,1)
    ...
    for usc_i = 1:length(unique_site_codes) % Loops through each site
        site_i = regexp(d.(city).site_names,unique_site_codes{usc_i}); % Array of all of the inlets at each site.
    ...
    if strcmp(plt.station_map,'y')
        ...
        if isempty(basemap_fn)
            ...
            plot_google_map('MapType','terrain','Scale',2,'showlabels',0,'APIKey','AIzaSyAuTqX_jToWk3gI6JhBGtAeHqdnfdhVkao') % redrawing the map to make sure it has the correct axes ratio.
            set(gca,'XLim',mapLim(1:2)); set(gca,'YLim',mapLim(3:4))

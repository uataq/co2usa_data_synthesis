...
% Must first run the CO2USA background processing script to generate all of the variables. 
...
fnStr = fullfile(writeFolder,city,[city,'_background_',species,'_1_hour_R0_',site.date_issued_str,'.nc']);

fprintf('Working on %s.\n',fnStr)

n.ncid = netcdf.create(fnStr,'NETCDF4');

varid = netcdf.getConstant('GLOBAL'); netcdf.putAtt(n.ncid,varid,'title',['Hourly background atmospheric ',site.unique_species_long_name{species_ind},' (',upper(species),') mixing ratios.']);
varid = netcdf.getConstant('GLOBAL'); netcdf.putAtt(n.ncid,varid,'summary',['Hourly background atmospheric ',upper(species),' mixing ratios from ',city_long_name,'.']);
...
varid = netcdf.getConstant('GLOBAL'); netcdf.putAtt(n.ncid,varid,'comment',['Observations represent the hourly ',upper(species),' mole fraction with the time stamp representing the floored hour. For example: data from 08:00 to 08:59 were averaged and have the time stamp of 08:00.']);
